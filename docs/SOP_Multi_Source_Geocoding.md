...
5.  **与智能校准自动降级的衔接**
    *   本节的 `Fallback` 仅指在正向地理编码候选中选择最高分结果，不等同于 POI 搜索。
    *   用户执行“智能编码”时，多源地理编码完成后会自动触发《智能校准功能 SOP》。
    *   没有选中结果、置信度缺失/异常或置信度低于90%的记录，必须进入 POI 自动降级流程，依次尝试百度、天地图和高德；地图 POI 仍未形成可信结果时，再进入网络信息和关键词搜索流程。
...
*   **流程**:
    1.  当 `TIANDITU_PROXY_URL` 与 `TIANDITU_PROXY_TOKEN` 均已配置时，通过 URPA 中转服务调用天地图正向地理编码，避免部署平台出口网络被天地图拒绝；未配置中转时才直接调用天地图API。
    2.  将服务商返回的原始结果直接返回给主瀑布流。
    3.  天地图逆地理编码采用相同的路由规则；中转请求不携带 Render 侧保存的天地图 Key，由 URPA 服务端注入自己的 Key。
    4.  批量任务中的天地图请求在应用端与 URPA 中转端均按全局 3 QPS（约每 334ms 一次）均匀排队，防止多个地址同时突发请求触发上游 HTTP 429；该速率已经过 12 次及 30 次持续真实请求验证。
