...
- 浏览器为每次 `/geocode/auto_select_point` 请求生成 `X-Request-ID`，后端日志及响应使用同一编号，便于关联前后端记录。
- 后端记录 `START`、`MODEL_DONE`、`SUCCESS` 或 `FAILED` 阶段及耗时；异常记录会进入管理后台报错中心。
- 如果浏览器未收到任何 HTTP 响应，前端会将请求编号、失败阶段、等待时长、在线状态和候选数量主动上报；服务短暂重启时会延迟重报一次。
- 模型供应商单次请求限制为 25 秒；普通用户的 GLM 只尝试一次，失败后立即调用一次 DeepSeek 灾备模型，避免触发 Gunicorn 的 120 秒 worker 超时。
- 当 GLM 返回供应商平台过载码 `1305` 时，当前进程开启 5 分钟熔断；熔断期内普通用户请求跳过 GLM 并直接调用 DeepSeek。账户自身限流 `1302` 或其他错误不会开启该熔断。
- 即使灾备模型成功，主模型异常也会以 `model_provider:<供应商>` 单独进入报错中心。记录仅包含模型名、异常类型、HTTP 状态码、供应商错误码、网页请求编号和供应商请求编号，不保存 API Key、Authorization、完整响应正文或提示词。
- `AUTO_SELECT MODEL_DONE` 日志会记录最终供应商、模型以及 `fallback_from`，可直接判断请求是否经过灾备切换。
