...
1.  **启动**: 功能被触发。
2.  **筛选目标**: 系统会遍历当前结果列表中的所有地址，并筛选出以下记录：
    - 没有选中结果（所有正向地理编码服务均未形成可用候选）；
    - 置信度缺失、无法解析或显示为 `-`；
    - 置信度低于90%。

   无结果或无置信度代表上游没有形成可信定位，其处理优先级不低于普通低置信度结果，必须进入后续 POI 自动降级流程。
...
2.  **二次尝试：天地图**
    - **操作**: 选中POI搜索源下拉框 (`#map-search-source`)中的“天地图”，然后模拟点击 **“<i class="bi bi-search"></i> 搜索”** 按钮 (`#mapSearchBtn`)。
    - **等待并判断**: 执行上述的 **“核心等待依据”** 和 **“核心成功依据”** 检查。不满足则进入下一步。

3.  **三次尝试：高德地图**
    - **操作**: 选中“高德地图”，再次点击 **“搜索”** 按钮。
    - **等待并判断**: 执行 **“核心等待依据”** 和 **“核心成功依据”** 检查。不满足则进入下一步。
